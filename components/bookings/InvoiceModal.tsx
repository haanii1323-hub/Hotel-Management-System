...
export default function InvoiceModal({ booking, collected, balance, onClose }: Props) {
  ...
  // Use persisted invoice number if it exists
  const invoiceNo =
    booking.invoices && booking.invoices.length > 0
      ? booking.invoices[0].invoiceNo
      : `INV-${(booking.bookingRef || '').replace('#', '')}-${(booking.id || '').slice(-4).toUpperCase()}`
      ...
      <div className="modal invoice-modal">
        ...
        <div className="invoice-header">
          ...
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontWeight: 700, fontSize: '16px' }}>TAX INVOICE</div>
        ...
        <div className="invoice-body">
          ...
          <div className="invoice-guest-section">
            <div>
              ...
              <div className="invoice-val" style={{ fontWeight: 600 }}>
            ...
            <tbody>
              <tr>
                ...
                <td style={{ textAlign: 'center' }}>
                  {nights} night{nights !== 1 ? 's' : ''} × {booking.numRooms} room{booking.numRooms !== 1 ? 's' : ''}
                </td>
                <td style={{ textAlign: 'right' }}>₹{(booking.nightlyRate || 0).toLocaleString('en-IN')}</td>
                <td style={{ textAlign: 'right' }}>₹{(booking.totalAmount || 0).toLocaleString('en-IN')}</td>
              </tr>
            </tbody>
          </table>
          ...
          <div className="invoice-totals">
            ...
            <div className="invoice-total-row">
              <span style={{ color: 'var(--green)' }}>Collected</span>
              ...
              {booking.payments.map((p: any, i: number) => (
                ...
                >
                  <span style={{ color: 'var(--text-2)' }}>
                    {p.mode} · {p.status} {p.utrRef ? `(UTR: ${p.utrRef})` : ''}
                  </span>
