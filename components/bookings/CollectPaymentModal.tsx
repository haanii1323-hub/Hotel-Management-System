...
export default function CollectPaymentModal({ booking, onClose, onSuccess }: Props) {
  ...
  // If any split row has UPI, compute UPI amount
  ...
  const upiString = upiId
    ? `upi://pay?pa=${upiId}&pn=${encodeURIComponent(payeeName)}&am=${upiAmount}&cu=INR&tn=Payment+${booking.bookingRef}`
    : ''
  ...
  async function handlePayment() {
    ...
    try {
      const res = await fetch(`/api/bookings/${booking.id}/payment`, {
        ...
        body: JSON.stringify({
          splits: validSplits.map((s) => ({
            ...
            notes: `Collected via PMS (${s.mode})`,
          })),
        }),
      })
      ...
      if (!res.ok) {
        ...
        const modesLabel = validSplits.map((s) => `${s.mode}: ${fmt(Number(s.amount))}`).join(', ')
        showToast(`Payment recorded: ${modesLabel}`, 'success')
        broadcastChange('PAYMENT_COLLECTED', { bookingId: booking.id, amount: totalSplitsAmount })
        onSuccess()
      }
    } catch {
    ...
    >
      <div className="modal" style={{ maxWidth: '580px', width: '94%' }}>
        ...
        <div className="modal-body">
            ...
            {splits.map((split, index) => (
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  {/* Auto-fill Helper (if multiple splits or remainder) */}
          ...
          >
            <div>
              <span style={{ color: 'var(--text-2)' }}>Total to Collect: </span>
            ...
            {remainingBalanceDiff === 0 ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: '5px', color: 'var(--green)', fontWeight: 600 }}>
                <CheckCircle2 size={14} /> Matches Outstanding Balance
              </div>
            ) : remainingBalanceDiff > 0 ? (
              <div style={{ color: 'var(--amber)', fontWeight: 600 }}>
                {fmt(remainingBalanceDiff)} Remaining Unallocated
              </div>
            ) : (
          ...
          {/* DYNAMIC UPI / QR VIEW (if UPI split exists) */}
          {upiSplit && (
            <div
              style={{
                background: 'var(--card-2)',
                ...
                <div style={{ display: 'flex', gap: '14px', alignItems: 'center' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    {config?.paymentInstructions && (
                      <div style={{ fontSize: '11px', color: 'var(--text-2)', marginTop: '4px', lineHeight: '1.3' }}>
                        {config.paymentInstructions}
                      </div>
                    )}
                  </div>
          ...
          {hasBankTransfer && config?.bankAccountNumber && (
            ...
            >
              <div style={{ fontWeight: 700, fontSize: '12px', marginBottom: '6px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                <Banknote size={14} color="var(--red)" /> Hotel Bank Account Details
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px' }}>
                <div><span style={{ color: 'var(--text-3)' }}>Beneficiary:</span> {config.bankAccountName || 'Hotel Account'}</div>
                <div><span style={{ color: 'var(--text-3)' }}>Bank:</span> {config.bankName || '—'}</div>
                <div><span style={{ color: 'var(--text-3)' }}>A/C:</span> <strong style={{ fontFamily: 'monospace' }}>{config.bankAccountNumber}</strong></div>
                <div><span style={{ color: 'var(--text-3)' }}>IFSC:</span> <strong style={{ fontFamily: 'monospace' }}>{config.bankIfsc || '—'}</strong></div>
              </div>
            </div>
          )}
        ...
        <div className="modal-footer">
          ...
          <button
            ...
            disabled={loading || totalSplitsAmount <= 0}
          >
            {loading ? (
              <span className="spinner" style={{ width: 14, height: 14 }} />
            ) : splits.length > 1 ? (
              `Record Split Payments (${fmt(totalSplitsAmount)})`
            ) : (
              `Record Payment (${fmt(totalSplitsAmount)})`
            )}
          </button>
        </div>
      </div>
